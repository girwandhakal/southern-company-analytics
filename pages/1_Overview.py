...
# Normalise standalone "Healthy" to the canonical label
...
# ═══════════════════════════════════════════════════════════════════════════
#  TABBED LAYOUT — merges original scaffold tabs with live data
# ═══════════════════════════════════════════════════════════════════════════
tab_exec, tab_lifecycle, tab_alerts = st.tabs(
    ["Executive Overview", "Lifecycle Status", "Recent Alerts"]
)

# ─── TAB 1: EXECUTIVE OVERVIEW (data-driven) ───────────────────────────────
with tab_exec:
    ...
    # Cost avoided by not replacing devices at decommissioned sites
    # that are already Critical or High risk
    ...
    # Left: Risk-level distribution bar chart
    with left:
    ...
    # Right: Top-10 high-risk states (Critical + High only)
    with right:
    ...
    # ── Full-width donut chart: Support Status ──────────────────────────
...
# ─── TAB 2: LIFECYCLE STATUS (enhanced from scaffold) ──────────────────────
with tab_lifecycle:
...
# ─── TAB 3: RECENT ALERTS (enhanced from scaffold) ─────────────────────────
with tab_alerts:
